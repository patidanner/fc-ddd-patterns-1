...
describe("Order repository test", () => {
  ...
  it("should find an order", async () => {
    ...
    const order = new Order("111", "123", [ordemItem]);

    const orderRepository = new OrderRepository();
    await orderRepository.create(order);

    const orderModel = await OrderModel.findOne({ where: { id: "111" }, include: ["items"], });

    const foundOrder = await orderRepository.find("111");
  ...
  it("should find all orders", async () => {
    ...
    const order2 = new Order("321", "123", [ordemItem]);
